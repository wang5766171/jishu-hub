...
/// Reads spec + plan from disk, runs the steps, writes result.
...
pub fn execute_plan_in_root(root: &Path, run_id: &str) -> Result<RunResult, String> {
    ...
    // Only allow executing plans that are currently in Complete status (Plan mode output)
    if result.status != RunStatus::Complete {
        return Err(format!(
            "Cannot execute plan: run {} is in {:?} status (expected Complete)",
    ...
    trace.append_event(&NormalizedEvent::TaskStep {
        ...
        title: "Plan execution started by user".to_string(),
        detail: Some(serde_json::json!({
            "steps": plan.len(),
        })),
    })?;
