...
fn status_from_args(args: Option<Vec<String>>) -> &'static str {
    match args {
        Some(args) if looks_like_hub_entry(&args) => "injected",
        Some(_) => "protected",
        None => "none",
    }
}

fn json_entry_args(config: &Value, servers_key: &str) -> Result<Option<Vec<String>>, String> {
    Ok(config
        .get(servers_key)
        .and_then(|s| s.get(HUB_MCP_ENTRY_NAME))
        .map(entry_args_of))
}

/// MCP 管理面状态（只读）：插件声明 + CLI 路径 + 四家条目归属
/// （injected=自家条目 / protected=用户自建同名 / none=未注入 / error）。
pub fn mcp_status() -> McpStatusReport {
    let mut report = McpStatusReport {
        plugins: load_mcp_plugin_decls()
            .into_iter()
            .map(|d| McpPluginInfo {
                id: d.plugin_id,
                command: d.command,
            ...
            .collect(),
        cli_path: resolve_cli_path().ok(),
        ..Default::default()
    };
    report.claude_code = crate::config::load_config()
        .map(|cfg| {
            status_from_args(
                cfg.mcp_servers
                    .as_ref()
    ...
    report.codex = crate::agent::adapters::codex::CodexAdapter::new()
        .load_config()
        .and_then(|v| json_entry_args(&v, "mcpServers").map(|a| status_from_args(a).to_string()))
        .unwrap_or_else(|e| format!("error: {e}"));
    report.opencode = crate::agent::adapters::opencode::OpencodeAdapter::new()
        .load_config()
        .and_then(|v| json_entry_args(&v, "mcpServers").map(|a| status_from_args(a).to_string()))
        .unwrap_or_else(|e| format!("error: {e}"));
    report.jishu_self = crate::agent::jishu_self::config::load_jishu_config()
        .map_err(|e| e.to_string())
        .and_then(|v| json_entry_args(&v, "mcp_servers").map(|a| status_from_args(a).to_string()))
        .unwrap_or_else(|e| format!("error: {e}"));
    report
}
