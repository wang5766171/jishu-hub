 ...
 *
 * 背景：task_launch 的 requirements/planning 阶段，LLM 需要读到 session_id 才能调
 * `advance_phase.mjs --session <id>`，脚本用 find_by_session 反查 task_id。原来 Hub
 ...
 * - session_id 取 pi 真实 id（ctx.sessionManager.getSessionId()），与 task_launch
 *   requirement_session_id（pi session id）一致，find_by_session 正常。
 *
...
const GUIDANCE =
  "该 session_id 由系统在每轮注入；阶段推进脚本需要当前会话时直接使用这个值，不要扫描 sessions 目录、猜测最新文件或自行推断。";
