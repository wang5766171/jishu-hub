...
本仓库面向多个 AI 智能体（Claude / OpenCode / Antigravity / Codex 等）。本文件为通用约定，各 agent 的专属配置（如 CLAUDE.md）以各自文件为准。
...
- **`PACKAGING.md`** —— 打包与发布指南：Windows 本地构建、macOS 云端自动化打包。

## Git 提交规范

执行 git 提交前，必须遵循 `agent-commit-guide.md`（本地文件，未跟踪）。要点：
- message 用中文；
- 以当前 agent 名称开头作前缀，如 `opencode:`、`codex:`、`antigravity:`、`claude:`；
