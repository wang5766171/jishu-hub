...
- `src-tauri/src/agent_runtime.rs`：统一选择 ACP/CLI transport。
- `src-tauri/src/acp_runtime.rs`：GUI 长会话 ACP client runtime。
...
必须由 adapter 或 adapter surface 提供的能力：

- agent 基础信息、健康检查、安装提示。
...
- `ModelStore { provider }`：适合 Jishu/Pi 模型库。
