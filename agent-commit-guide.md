...
- **每一次提交都需要用户当次明确允许**。用户此前允许过提交、或允许过某个批次的提交，均**不构成**后续提交的授权。
- 标准流程：agent 完成改动 → 用户测试验证 → 用户明确指示提交（如"提交"/"没问题了，提交"）→ agent 才执行 `git commit`。
- 用户未允许前，改动保留在工作区，不主动 commit（更不 push）。
- 同一主题的多轮小调整（如 UI 微调）应累积为**一个**提交，经用户许可后一次提交，避免"一改一交"。
