 ...
 * 运行时强制开关——生产构建同样启用日志中心。后端 settings.json
 * （devLogForced）为持久化权威，启动时水合（见下）。dev 构建恒启用。
 */
...
/** 启动时从后端 settings.json 水合（app 挂载早期调用一次）。水合前
 *  isDevLogForced() 返回 false（短暂闪断可接受——localStorage 跨启动
 *  丢失已被后端持久化解决，不再使用 localStorage）。 */
