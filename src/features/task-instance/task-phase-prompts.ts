...
export function buildRequirementsStagePrompt({
  ...
  return [
    ...
    `调用方式：node ~/.jishu-agent/task-plan/jishu-task-planner/scripts/advance_phase.mjs --phase "planning" --project "${project}" --requirement-file "<需求终稿 markdown 文件>" --session "<当前会话ID>"`,
...
export function buildPlanningStagePrompt({
  ...
  return [
    ...
    `调用方式：node ~/.jishu-agent/task-plan/jishu-task-planner/scripts/advance_phase.mjs --phase "execution" --project "${project}" --session "<当前会话ID>"`,
