...
export const ShellExecBody = memo(function ShellExecBody({ input, output, error }: { input: Record<string, unknown>; output?: string; error?: string }) {
  ...
  return (
    <div className="text-[0.75em] space-y-1.5">
      ...
      {output && (
        <pre className={`font-mono text-[0.73em] bg-[var(--tool-card-code-bg)] border border-border/45 rounded-lg p-2 overflow-x-auto max-h-40 overflow-y-auto whitespace-pre ${error ? "text-[var(--tool-error)]" : ""}`}>
          {output.length > 3000 ? output.slice(0, 3000) + "\n... (truncated)" : output}
        </pre>
      )}
      {error && (
        <div className="text-[0.73em] text-[var(--tool-error)] font-mono bg-red-500/10 border border-[var(--tool-error)]/25 rounded-lg p-2">
          {error}
        </div>
      )}
    </div>
