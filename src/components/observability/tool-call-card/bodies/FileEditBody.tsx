  ...
  return (
    <div className="space-y-2 text-[0.95em]">
      ...
      {diff ? (
        ...
        <pre className="max-h-56 overflow-auto whitespace-pre rounded-[4px] border border-border/45 bg-[var(--tool-card-code-bg)] p-2.5 font-mono text-[0.95em]">
          {readable.length > 2400 ? `${readable.slice(0, 2400)}\n... (truncated)` : readable}
        </pre>
      ) : output ? (
        <pre className="max-h-48 overflow-auto whitespace-pre rounded-[4px] border border-border/45 bg-[var(--tool-card-code-bg)] p-2.5 font-mono text-[0.95em]">
...
function InlineDiff({ diff }: { diff: DiffPreview }) {
  return (
    <div className="overflow-hidden rounded-[4px] border border-border/50 bg-background/45">
